...
==========================
Direct pipeline test â€” NO HTTP endpoints, NO WhatsApp, NO UI.
...
Leave as None to skip GST lines â€” the test still validates the voucher path.
"""
...
# â”€â”€ Path fixup so the script can be run from backend/ or repo root â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â–º CONFIG â€” edit these to match your Tally setup
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def _post_xml(xml_payload: str, label: str) -> tuple[bool, str]:
    ...
    """
    print(f"  Sending {len(xml_payload):,} bytes to {TALLY_URL} â€¦")
    try:
        resp = requests.post(
            TALLY_URL,
        ...
        msg = (f"Cannot connect to Tally at {TALLY_URL}\n"
               "  â†’ Make sure Tally is open and the XML Server is enabled on port 9000.")
        return False, msg
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TEST 1 â€” App Pipeline: SALES via GoldenXMLBuilder
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def test_1_golden_sales() -> tuple[bool, str]:
    ...
    data = VoucherData(
        ...
        narration="Pipeline test â€” Sales via GoldenXMLBuilder",
    ...
    buf = io.StringIO()
    buf.write(f"  Item: â‚¹{RATE*QTY:.2f}  GST: â‚¹{gst_amt if GST_LEDGER_SALES else 0:.2f}  Total: â‚¹{grand:.2f}\n")
    if not GST_LEDGER_SALES:
        buf.write("  (GST_LEDGER_SALES not set â€” item-only voucher)\n")
    buf.write(f"  XML builder : GoldenXMLBuilder.build_sales_voucher()\n")
    buf.write(f"  SVCURRENTCOMPANY injected: {'YES' if 'SVCURRENTCOMPANY' in xml else 'NO â€” uses active company'}\n")
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TEST 2 â€” App Pipeline: PURCHASE via GoldenXMLBuilder
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def test_2_golden_purchase() -> tuple[bool, str]:
    ...
    data = VoucherData(
        ...
        narration="Pipeline test â€” Purchase via GoldenXMLBuilder",
        inventory_items=[InventoryItem(name=ITEM, quantity=QTY, rate=RATE,
    ...
    buf = io.StringIO()
    buf.write(f"  Item: â‚¹{RATE*QTY:.2f}  GST: â‚¹{gst_amt if GST_LEDGER_PURCHASE else 0:.2f}  Total: â‚¹{grand:.2f}\n")
    if not GST_LEDGER_PURCHASE:
        buf.write("  (GST_LEDGER_PURCHASE not set â€” item-only voucher)\n")
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TEST 3 â€” WhatsApp Pipeline: SALES via build_invoice_xml
#
...
#   â€¢ Line ~515: @dataclass with __init__
#   â€¢ Line ~545: plain class â€” no __init__ (shadows the dataclass)
...
def test_3_whatsapp_sales() -> tuple[bool, str]:
    ...
    vch_fields = {
        ...
        "NARRATION": "Pipeline test â€” Sales via WhatsApp builder",
    ...
    buf = io.StringIO()
    buf.write(f"  Item: â‚¹{item_total:.2f}  GST: â‚¹{gst_amt if GST_LEDGER_SALES else 0:.2f}  Total: â‚¹{grand:.2f}\n")
    if not GST_LEDGER_SALES:
        buf.write("  (GST_LEDGER_SALES not set â€” item-only voucher)\n")
    buf.write(f"  XML builder : build_invoice_xml() from tally_xml_builder.py\n")

    has_svc = "SVCURRENTCOMPANY" in xml
    buf.write(f"  SVCURRENTCOMPANY injected: {'YES â† known issue' if has_svc else 'NO'}\n")
    if has_svc:
        buf.write(
            "  âš ï¸  Pipeline difference: WhatsApp builder sends <SVCURRENTCOMPANY> but Tally\n"
            "     rejects it unless the company is open with that EXACT name.  App pipeline\n"
            "     (GoldenXMLBuilder) omits the tag â€” that's why TEST 1/2 pass and TEST 3/4 fail.\n"
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TEST 4 â€” WhatsApp Pipeline: PURCHASE via build_invoice_xml
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def test_4_whatsapp_purchase() -> tuple[bool, str]:
    ...
    vch_fields = {
        ...
        "PARTYLEDGERNAME": PARTY, "PARTYNAME": PARTY,
        "NARRATION": "Pipeline test â€” Purchase via WhatsApp builder",
    ...
    buf = io.StringIO()
    buf.write(f"  Item: â‚¹{item_total:.2f}  GST: â‚¹{gst_amt if GST_LEDGER_PURCHASE else 0:.2f}  Total: â‚¹{grand:.2f}\n")
    if not GST_LEDGER_PURCHASE:
        buf.write("  (GST_LEDGER_PURCHASE not set â€” item-only voucher)\n")
    buf.write(f"  XML builder : build_invoice_xml() from tally_xml_builder.py\n")
    ...
    buf.write(f"  SVCURRENTCOMPANY injected: {'YES â† known issue' if has_svc else 'NO'}\n")
    if has_svc:
        buf.write("  âš ï¸  Same SVCURRENTCOMPANY issue as TEST 3 â€” see above.\n")
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# Entry point â€” run tests sequentially, print buffered results
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

TESTS = [
    ("TEST 1 â€” App Pipeline SALES    (GoldenXMLBuilder)",    test_1_golden_sales),
    ("TEST 2 â€” App Pipeline PURCHASE (GoldenXMLBuilder)",    test_2_golden_purchase),
    ("TEST 3 â€” WhatsApp SALES        (build_invoice_xml)",   test_3_whatsapp_sales),
    ("TEST 4 â€” WhatsApp PURCHASE     (build_invoice_xml)",   test_4_whatsapp_purchase),
]
...
if __name__ == "__main__":
    ...
    print(f"  Party       : {PARTY}")
    print(f"  Item        : {ITEM}  Qty={QTY}  Rate=â‚¹{RATE}  GST={GST_PCT}%")
    print(f"  Date        : {TODAY}")
    print(f"  GST (Sales) : {GST_LEDGER_SALES or 'â€” skipped'}")
    print(f"  GST (Purch) : {GST_LEDGER_PURCHASE or 'â€” skipped'}")
    print(SEP)
    ...
    for name, fn in TESTS:
        ...
        icon = "âœ… PASS" if passed else "âŒ FAIL"
        print(f"\n  {icon}")
        print(detail)
    ...
    if passed_count < len(results):
        ...
        print("  1. GST ledger names don't exist â†’ set GST_LEDGER_SALES / GST_LEDGER_PURCHASE in the script")
        print("  2. WhatsApp pipeline (TEST 3/4) sends <SVCURRENTCOMPANY> â†’ Tally rejects it")
        print("     Fix: remove SVCURRENTCOMPANY from _wrap_envelope() in tally_xml_builder.py")
        print("  3. InventoryEntry defined twice in tally_xml_builder.py (bug) â†’ handled via _FakeInventoryEntry")
